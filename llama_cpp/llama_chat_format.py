...
class Llava15ChatHandler:
    ...
    "{%- endif %}\n"
    "\n"
    # pull out system message if it's the first one
    ...
    "      {{ message['content'].replace('<|reserved_special_token_69|>', '').lstrip() }}"
    "      {{ '<|eot_id|>' }}\n"
    ...
    "        {%- elif content.type == 'image_url' and content.image_url is string %}"
    "          {{ content.image_url }}\n"
